...
// Functions for calling trap handling in trapentry.S
void DIVIDE();
void DEBUG();
void NMI();
void BRKPT();
...
trap_init_percpu(void)
{
	...
	uint8_t cpu_num = thiscpu->cpu_id;
	uint64_t kstacktop_i = KSTACKTOP - cpu_num * (KSTKSIZE + KSTKGAP);
	...
	ltr(GD_TSS0 + cpu_num * sizeof(struct Segdesc));
...
trap_dispatch(struct Trapframe *tf)
{
	...
	switch(tf->tf_trapno) {
		...
		case T_PGFLT:
			...
			return;
		case T_SYSCALL:			
			result = syscall(tf->tf_regs.reg_eax, 
					tf->tf_regs.reg_edx, 
					tf->tf_regs.reg_ecx, 
					tf->tf_regs.reg_ebx, 
					tf->tf_regs.reg_edi, 
